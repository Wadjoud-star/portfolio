#!/usr/bin/env python3
"""Génère assets/cv-winamax-search.pdf — CV Winamax Paris (search/pertinence, React/Redux, Redis)."""
from pathlib import Path

OUT = Path(__file__).resolve().parent.parent / "assets" / "cv-winamax-search.pdf"
...
def main():
    ...
    p.y -= 14
    p.text(m, 10, "Eleve Ingenieur - React / TypeScript / Redux / Node / Redis", bold=True, color=ACCENT)
    p.y -= 11
    p.text(
        m, 8.2,
    ...
    p.multilines(
        7.8,
        "Eleve ingenieur bac+5 (ESIGELEC), developpeur Full Stack centre sur React / TypeScript / Redux "
        "et Node.js. Je soigne la pertinence des resultats (ranking, reduction du bruit, filtres), "
        "la lisibilite UX et la performance temps reel (cache Redis, LRU local, catalogues a forte "
        "frequence de mise a jour). Capable de reprendre un moteur existant sur le fond (algo) et "
        "sur la forme (ergonomie, hierarchisation). Recherche un stage Master 2 a temps plein a Paris "
        "a partir de fevrier 2027 - ouvert a un CDI.",
        leading=9.6,
        color=MUTED,
    )
    ...
    skills = [
        ("Frontend", "React, TypeScript, Redux, UI recherche, filtres dynamiques, suggestions, ergonomie"),
        ("Backend", "Node.js, JavaScript, API temps reel, ranking / scoring, filtrage serveur"),
        ("Cache & Perf", "Redis, cache LRU (local / serveur), latence, catalogues volumineux, debounce"),
        ("Qualite", "Algorithmique, hierarchisation de l'info, tests, Git, Docker, mesure de pertinence"),
    ]
    ...
    projects = [
        (
            "Club Sport - Recherche & hierarchisation de resultats",
            "2025",
            [
                "Moteur de recherche multi-criteres (clubs, sports, localisation) sur catalogue structure",
                "Filtrage, tri et lisibilite des resultats - reduction du bruit et clarte UX",
                "Performance cote app + base SQL - parcours consultes frequemment",
            ],
            "Java | MySQL | Algorithmique | UI recherche | Docker | Git",
        ),
        (
            "Factis - SPA React/TS + filtres dynamiques & dashboards",
            "2025",
            [
                "Interfaces React/TypeScript avec filtres, tri et etat d'affichage (proche Redux)",
                "Listes volumineuses : pertinence visuelle, ranking metier, reactivite UI",
                "API Node-like + PostgreSQL - attention latence et experience temps reel",
            ],
            "React | TypeScript | Next.js | Prisma | PostgreSQL | Docker | Git",
        ),
        (
            "Beniphone - Catalogue, suggestions & historique de consultation",
            "2025",
            [
                "Parcours de recherche / navigation dans un catalogue multi-entites",
                "Historique des actions utilisateur et etats de consultation (tendances d'usage)",
                "Back React + API Node.js / MySQL - outils utilisables en continu",
            ],
            "React | TypeScript | Node.js | Express | MySQL | Git",
        ),
        (
            "Smart CMS IA - Cache, jobs & performance",
            "2025",
            [
                "Pipelines asynchrones, etats caches, mesure de completion / echec",
                "Approche perf-first : ce qui n'est pas assez rapide n'est pas viable",
            ],
            "Next.js | TypeScript | Node.js | Redis (notions) | Supabase | Git",
        ),
        (
            "Mon Demenagement - UX lisible & iteration produit",
            "2025",
            [
                "Amelioration de la lisibilite des parcours et de la hierarchisation de l'information",
            ],
            "JavaScript | PHP | MySQL | HTML/CSS | Git",
        ),
    ]
    ...
    p.multilines(
        7.4,
        "React, TypeScript, Redux, Node.js, Redis, algorithmique, UX, performance front, SQL, Agile",
        leading=9.4,
        color=MUTED,
    )
    p.space(2)
    ...
    p.y -= 10
    p.bullet("Rigueur, autonomie et sens du detail sur des outils utilises au quotidien")

    p.section("Langues, disponibilite et focus stage")
    ...
    p.space(2)
    p.text(m, 8, "Ce que je veux livrer chez Winamax", bold=True, color=TEXT)
    p.y -= 10
    p.multilines(
        7.5,
        "Moins de bruit, meilleur ranking  |  Filtres dynamiques  |  Suggestions tendances + historique LRU  |  "
        "UX lisible  |  Perf temps reel (Redis / React / Redux)",
        leading=9.4,
        color=MUTED,
    )
