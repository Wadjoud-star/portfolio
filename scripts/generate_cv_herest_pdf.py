#!/usr/bin/env python3
"""Génère assets/cv-herest.pdf — CV Herest Marcq-en-Baroeul (Data Engineer / Full-Stack)."""
from pathlib import Path

OUT = Path(__file__).resolve().parent.parent / "assets" / "cv-herest.pdf"
...
def main():
    ...
    p.y -= 14
    p.text(m, 10, "Eleve Ingenieur - Data Engineer / Full-Stack (Python / SQL)", bold=True, color=ACCENT)
    p.y -= 11
    p.text(
        m, 8.2,
        "Stage 6 mois+ (janv. 2027) - Mobilite Marcq-en-Baroeul - ESIGELEC Rouen",
        color=MUTED,
    )
    p.y -= 10
    ...
    p.multilines(
        7.8,
        "Eleve ingenieur bac+5 (ESIGELEC), developpeur Full Stack avec une premiere experience concrete "
        "en Python et SQL (jointures, agregations, sous-requetes / CTE). Je fiabilise des traitements "
        "de donnees, mets en place des controles qualite, documente les anomalies et contribue a des "
        "outils internes (scripts, petits endpoints, ecrans simples). Rigoureux, curieux, a l'aise pour "
        "comprendre les besoins metier et faire evoluer du code existant avec Git. Recherche un stage "
        "de 6 mois minimum a Marcq-en-Baroeul a partir de janvier 2027.",
        leading=9.6,
        color=MUTED,
    )
    ...
    skills = [
        ("Data & Python", "Python (scripts structures), transformation de donnees, validations, logs, automatisation"),
        ("SQL", "MySQL / PostgreSQL : jointures, agregations, sous-requetes, CTE, modelisation, coherence outputs"),
        ("Full Stack", "API / petits endpoints (Node.js, Python), ecrans internes (React/TS), import/export fichiers"),
        ("Qualite & Outils", "Jeux de test, diagnostic d'anomalies, documentation, Git, Docker, revue de code"),
    ]
    ...
    projects = [
        (
            "Factis - Flux data, SQL & reportings metier",
            "2025",
            [
                "Modelisation SQL (jointures, agregations), imports / exports et coherence des outputs",
                "Controles sur les donnees metier (clients, factures, analytics) et documentation des ecarts",
                "Ecrans internes + API : qualification des parcours et fiabilisation des traitements",
            ],
            "Python (scripts) | SQL / PostgreSQL | TypeScript | Prisma | Docker | Git",
        ),
        (
            "Smart CMS IA - Automatisation & qualite de traitements",
            "2025",
            [
                "Scripts / jobs asynchrones, validations et logs sur les pipelines de contenu",
                "Jeux de tests, reproduction d'anomalies, hypothese de correction et criticite",
                "Petits endpoints et ecrans de suivi pour fiabiliser les workflows",
            ],
            "Python | TypeScript | Node.js | SQL | Supabase | Git",
        ),
        (
            "Beniphone - Plateforme interne multi-roles",
            "2025",
            [
                "Transformation de donnees metier, etats traces, outils pour utilisateurs non techniques",
                "Diagnostic d'anomalies (source, impact) et evolution de code existant",
            ],
            "Python (bases) | Node.js | MySQL | React | Git",
        ),
        (
            "Club Sport - Dependances data / interfaces / utilisateurs",
            "2025",
            [
                "Comprehension des flux (sources, traitements, ecrans), roles et exports",
                "Tests fonctionnels des parcours et verification de coherence des resultats",
            ],
            "SQL | Java | MySQL | Docker | Git",
        ),
        (
            "Mon Demenagement - Outils internes & maintenance",
            "2025",
            [
                "Amelioration de traitements fichier / reporting, support et documentation",
            ],
            "PHP | MySQL | SQL | JavaScript | Git",
        ),
    ]
    ...
    p.multilines(
        7.4,
        "Python, SQL (jointures, agregations, CTE), Git, Docker, API, ecrans web, qualite logicielle / data",
        leading=9.4,
        color=MUTED,
    )
    p.space(2)
    ...
    p.y -= 10
    p.bullet("Rigueur, curiosite et implication : diagnostic, support, communication avec les utilisateurs")
    p.bullet("Autonomie progressive tout en sollicitant l'equipe en cas de besoin")
    ...
    p.multilines(
        7.5,
        "Francais : langue maternelle  |  Anglais : B2 (docs techniques)",
        leading=9.4,
        color=MUTED,
    )
    p.space(2)
    ...
    p.multilines(
        7.5,
        "Stage 6 mois minimum a partir de janvier 2027  |  Mobilite Marcq-en-Baroeul (59700)",
        leading=9.4,
        color=MUTED,
    )
    p.space(2)
    ...
    p.multilines(
        7.5,
        "Python & SQL  |  Qualite data / tests  |  Automatisation  |  Outils internes  |  Git & code existant",
        leading=9.4,
        color=MUTED,
    )
