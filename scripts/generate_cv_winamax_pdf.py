#!/usr/bin/env python3
"""Génère assets/cv-winamax.pdf — CV Winamax Paris (React/TS, Node, temps réel, édito)."""
from pathlib import Path

OUT = Path(__file__).resolve().parent.parent / "assets" / "cv-winamax.pdf"
...
def main():
    ...
    p.y -= 14
    p.text(m, 10, "Eleve Ingenieur - Developpeur React / TypeScript / Node.js", bold=True, color=ACCENT)
    p.y -= 11
    p.text(
        m, 8.2,
        "Stage fin d'etudes Master 2 (fev. 2027) - Temps plein Paris - ESIGELEC Rouen",
        color=MUTED,
    )
    p.y -= 10
    ...
    p.multilines(
        7.8,
        "Eleve ingenieur bac+5 (ESIGELEC), developpeur Full Stack centre sur React / TypeScript et Node.js. "
        "Je construis des interfaces interactives performantes, des back-offices pour des equipes non tech, "
        "et des APIs fiables (MySQL, Redis, etats utilisateur). Attentif aux cas limites (UX, gestes, "
        "reseau degrade), a l'instrumentation produit (ouvertures, completion, decrochage) et au dialogue "
        "direct avec produit / creatif / editorial. Motive par les produits a forte audience et le temps "
        "reel. Recherche un stage de fin d'etudes a temps plein a Paris a partir de fevrier 2027 - ouvert CDI.",
        leading=9.6,
        color=MUTED,
    )
    ...
    skills = [
        ("Frontend", "React, TypeScript, JavaScript, composants interactifs, media / video UI, etats UX"),
        ("Backend", "Node.js, JavaScript, JSDoc, API REST, WebSocket / temps reel (notions), auth"),
        ("Data & Cache", "MySQL, Redis, modelisation d'etats (vu / pas vu / en cours), instrumentation"),
        ("Produit & Qualite", "Back-office metier, maquettes -> propositions UX, Git, Docker, perf mobile"),
    ]
    ...
    projects = [
        (
            "Beniphone - Contenu interactif + back-office metier",
            "2025",
            [
                "Ecrans React multi-roles : diffusion de contenus, moderation, parcours utilisateurs",
                "Outils pour equipes non techniques (planification / validation) sans passer par un dev",
                "Etats de consultation et workflows traces (vu, en cours, valide) - MySQL + API Node",
            ],
            "React | TypeScript | Node.js | Express | Prisma | MySQL | Git",
        ),
        (
            "Factis - SPA React/TS performante + analytics metier",
            "2025",
            [
                "Composants interactifs, dashboards et suivi d'usage (ouvertures / parcours)",
                "Back-office de gestion (clients, factures) pense pour des utilisateurs metier",
                "Attention perf UI, transitions et experience sur ecran principal de l'app",
            ],
            "React | Next.js | TypeScript | Prisma | PostgreSQL | Docker | Git",
        ),
        (
            "Smart CMS IA - Media, jobs async & instrumentation",
            "2025",
            [
                "Pipeline de contenus (generation, publication) avec suivi d'etat par entite",
                "Jobs asynchrones, APIs Node/TS, mesures de completion / echec des traitements",
                "Propositions UX sur les cas limites et documentation claire",
            ],
            "Next.js | React | TypeScript | Node.js | OpenAI | Supabase | Git",
        ),
        (
            "Club Sport - Etats utilisateur & droits multi-roles",
            "2025",
            [
                "Modelisation fine des droits et des etats metier (consultation, exports, roles)",
                "Application structuree, fiabilite des donnees, deploiement Docker",
            ],
            "Java | MySQL | Docker | Git | Agile",
        ),
        (
            "Mon Demenagement - Prod, UX & collaboration produit",
            "2025",
            [
                "Evolution d'une plateforme en production : parcours, lisibilite, retours utilisateurs",
                "Contact direct metier / produit pour prioriser et livrer",
            ],
    ...
    p.multilines(
        7.4,
        "React, TypeScript, Node.js, MySQL, Redis (notions), WebSocket, UX, performance front, Agile",
        leading=9.4,
        color=MUTED,
    )
    p.space(2)
    ...
    p.y -= 10
    p.bullet("Autonomie, sens produit et communication claire avec des interlocuteurs non techniques")

    p.section("Langues, disponibilite et motivation")
    p.text(m, 8, "Langues", bold=True, color=TEXT)
    p.y -= 10
    p.multilines(
        7.5,
        "Francais : langue maternelle  |  Anglais : B2 (docs, specs, echanges techniques)",
        leading=9.4,
        color=MUTED,
    )
    p.space(2)
    ...
    p.multilines(
        7.5,
        "Stage fin d'etudes Master 2 a temps plein a Paris a partir de fevrier 2027  |  Ouvert a un CDI",
        leading=9.4,
        color=MUTED,
    )
    p.space(2)
    p.text(m, 8, "Ce que je veux construire chez Winamax", bold=True, color=TEXT)
    p.y -= 10
    p.multilines(
        7.5,
        "Format riche & interactif (video, reactions temps reel)  |  Back-office editorial autonomisant  |  "
        "Etats de consultation fiables  |  Mesure d'engagement  |  UX solide hors cas nominaux",
        leading=9.4,
        color=MUTED,
    )
